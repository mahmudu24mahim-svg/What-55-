...
async def get_number(update, context):
    ...
    await update.message.reply_text("🔢 Now, enter the amount (Max: 10).")
    return GETTING_AMOUNT

async def get_amount_and_process(update, context):
    amount_str = update.message.text
    if not amount_str.isdigit() or not (0 < int(amount_str) <= 10):
        await update.message.reply_text("❌ Invalid amount. Please enter a number between 1 and 10.")
        return GETTING_AMOUNT
